...
func New(tblGenerator table.Generator) EndcoderDecoder {
	return EndcoderDecoder{tblGenerator: tblGenerator}
}

func (ed EndcoderDecoder) Encode(str string) []byte {
	tbl := ed.tblGenerator.NewTable(str)

	encode := encodeBin(str, tbl)

	return buildEncodedFile(tbl, encode)
}

func (ed EndcoderDecoder) Decode(encodedData []byte) string {
...
// encodeBin encodes str into binary codes string whitout spaces.
